...
Route::group(['prefix' => 'dashboard/support/', 'as'=>'support.', 'middleware' => ['auth', 'verified']], function() {
    ...
    Route::group(['as'=>'category.', 'prefix'=>'category'], function($routes){
        $routes->get('/', [CategoryController::class, 'index'])->name('index');
        $routes->post('/', [CategoryController::class, 'store'])->name('store');
        $routes->get('/edit/{id}', [CategoryController::class, 'edit'])->name('edit');
        $routes->post('/update', [CategoryController::class, 'update'])->name('update');
        $routes->get('/delete/{id}', [CategoryController::class, 'destroy'])->name('destroy');
    });
