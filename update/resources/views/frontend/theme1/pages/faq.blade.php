                        ...
                        <div class="col-lg-12">
                            <div class="tab-content">
                                <div class="tab-pane fade show active" id="aiImgGenerateTab">
                                    @if (!empty($faqs))
                                        @foreach ($faqs as $faq)
                                            <div class="accordion custom-accordion custom-accordion--faq mb-8"
                                                id="faqAccordionOne">

                                                <div class="accordion-item border-bottom-0 border-start-0 border-end-0">
                                                    <h2 class="accordion-header">
                                                        <button class="accordion-button fs-20 fw-bold" type="button"
                                                            data-bs-toggle="collapse" data-bs-target="#faqAccordion{{ $faq->id }}"
                                                            aria-expanded="true"> {{ $faq->collectLocalization('question') }} </button>
                                                    </h2>
                                                    <div id="faqAccordion{{ $faq->id }}" class="accordion-collapse collapse {{ $loop->iteration == 1 ? 'show' : '' }}"
