                    ...
                    <div class="tt-page-header">
                        <div class="d-lg-flex align-items-center justify-content-lg-between">
                            <div class="tt-page-title mb-3 mb-lg-0">
                                <h1 class="h4 mb-lg-1">{{ localize('Words Report') }}</h1>
                                <ol class="breadcrumb breadcrumb-angle text-muted">
                                    <li class="breadcrumb-item"><a
                                            href="{{ route('writebot.dashboard') }}">{{ localize('Dashboard') }}</a>
                                    </li>
                                    <li class="breadcrumb-item">{{ localize('Words Report') }}</li>
                                </ol>
                            </div>
                            <div class="tt-action">
                                        ...
                                        <div class="input-group">
                                            <select class="form-select select2" name="template_id">
                                                ...
                                                @foreach ($templates as $template)                                                
                                                    <option value="{{$template->id}}" {{isset($template_id) ? ($template_id ==  $template->id ? 'selected':''):''}}> {{$template->name}}
                                                       </option>
                                                @endforeach
                        ...
                        <table class="table tt-footable border-top align-middle" data-use-parent-width="true">
                            <thead>
                                <tr>
                                    ...
                                    <th data-breakpoints="xs">{{ localize('Template') }}</th>
                            ...
                            <tbody>
                                @foreach ($usage as $key => $use)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="fw-bold">
                                                {{ localize($use->template->collectLocalization('name')) }}
                                            </span>
                                        </td>
                                        <td>
                                                {{ $use->user->name }}
                                        ...
                                        <td class="text-end">
                                            <span class="fw-bold">{{ formatWords($use->total_used_words) }}</span>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
