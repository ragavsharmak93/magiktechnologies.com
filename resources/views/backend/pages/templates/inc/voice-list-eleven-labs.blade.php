<table class="table tt-footable align-middle" data-use-parent-width="true">
    <thead>
        <tr>
            ...
            <th data-breakpoints="xs sm">{{ localize('Voice') }}</th>
        ...
        @if(!empty($voiceLists))
            @foreach ($voiceLists as $key => $voice)
                <tr>
                    ...
                    <td>{{ @$voice->elevenLabVoice->name }} [{{@$voice->elevenLabVoice->accent}}] [{{@$voice->elevenLabVoice->description}}] [{{@$voice->elevenLabVoice->use_case}}]</td>
