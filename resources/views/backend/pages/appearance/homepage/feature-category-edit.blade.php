        ...
        <div class="container">
            ...
            <div class="row mb-4 g-4">
                ...
                <div class="col-xl-9 order-2 order-md-2 order-lg-2 order-xl-1">
                    ...
                    <form action="{{ route('admin.appearance.homepage.update-feature-category') }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id" value="{{$editFeatureCategory->id}}">
                        <input type="hidden" name="language_key" id="language_id" value="{{ @$lang_key }}">
                        <!--slider info start-->
                        <div class="card mb-4">
                            <div class="card-body">
                                <h5 class="mb-4">{{ localize('Edit Feeback Category') }}</h5>

                                <div class="mb-3">
                                    <label for="name" class="form-label">{{ localize('Name') }}</label>
                                    <input type="text" name="name" id="name"
                                        placeholder="{{ localize('Type name') }}" class="form-control" value="{{$editFeatureCategory->collectLocalization('name')}}" required>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="icon" id="icon"
                                        value="{{ $editFeatureCategory->icon }}"
